...
def write_report(events, counts, rank_frame, grad_frame, checkpoint_frame, plateau):
    ...
    if best is None:
        (RUN_OUTPUT / 'DRPA8_FULL_DATA_TRAINING_REPORT.md').write_text(
            ...
            '- Validation points: `0` (validation interval is 500 steps)\n'
    ...
    finite_grad = int(grad_frame[['lora_finite_grad_tensors', 'projection_finite_grad_tensors', 'decoder_finite_grad_tensors']].min().min()) if not grad_frame.empty else 0
...
- Minimum finite gradient-tensor count observed: `{finite_grad}`.
