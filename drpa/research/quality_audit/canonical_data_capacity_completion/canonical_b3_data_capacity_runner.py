#!/usr/bin/env python3
"""Canonical B3-Full FP32 fixed-step data-capacity training; 81,930,624 params."""
...
BASE=Path(os.environ.get('MTL_MODEL_ROOT', '__DRPA_WORKSPACE__')); AUDIT=BASE/'quality_audit/drpa_data_capacity_scaling'; PILOT=BASE/'quality_audit/voxtell_mtl_drpa8_pilot'; PEFT=BASE/'quality_audit/voxtell_mtl_peft'; PREP=BASE/'quality_audit/voxtell_mtl_b1_bilateral_crop'; B3=BASE/'quality_audit/voxtell_full_data_b3_vs_drpa_efficiency'
B3BASE=BASE/'quality_audit/voxtell_mtl_b3_decoder_capacity_upper_bound'
VOXTELL=BASE/'VoxTell'
sys.path[:0]=[str(PILOT),str(PEFT),str(PREP),str(B3BASE),str(B3),str(VOXTELL)]
...
SEED=20260809; DEVICE=torch.device('cuda'); MODEL=str(BASE/'VoxTell_weights/voxtell_v1.1'); BANK=str(BASE/'VoxTell_weights/embeddings/voxtell_v1.1/text_embeddings.npz'); CACHE=str(PEFT/'text_embedding_cache.npz'); EXPECTED={'b3':81930624}
...
def make_wrapper(kind):
    if kind != 'b3': raise RuntimeError('canonical runner permits only B3-Canonical')
    w=B3FullDataWrapper(MODEL,BANK,rank=4,alpha=8,dropout=.05,device=DEVICE); w.set_training_mode()
    groups={'cross_attention_lora':[],'projection_adapter':[],'decoder_stages':[]}
...
def validate(w,val,val_ds,cache,step,out,kind,train_len):
    ...
    return r
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--model',choices=['b3'],required=True); ap.add_argument('--subset',choices=['10pct','25pct','50pct'],required=True); ap.add_argument('--max-steps',type=int,default=6000); ap.add_argument('--output-dir',required=True); a=ap.parse_args(); out=Path(a.output_dir); out.mkdir(parents=True,exist_ok=True); (out/'checkpoints').mkdir(exist_ok=True)
    refuse_busy_gpu(); seed_all(); train_path=AUDIT/'manifests'/f'train_{a.subset}.csv'; val_path=AUDIT/'manifests/val_100pct_frozen.csv'; train_frame=pd.read_csv(train_path,dtype=str); val_frame=pd.read_csv(val_path,dtype=str); train=records(train_path); val=records(val_path); spec=load_crop_spec(PILOT/'crop_spec.json'); train_ds=BilateralGroupedPatchDataset(train,spec,cache_cases=False); val_ds=BilateralGroupedPatchDataset(val,spec,cache_cases=False); cache=TextEmbeddingCache(BANK,MODEL,CACHE); w,groups,save=make_wrapper(a.model); counts={g:sum(p.numel() for p in ps) for g,ps in groups.items()}; config={'experiment':'B3_CANONICAL_DATA_CAPACITY','model':'b3_canonical','subset':a.subset,'seed':SEED,'max_optimizer_steps':a.max_steps,'train_visits':len(train),'train_ptids':int(train_frame.ptid.nunique()),'val_visits':len(val),'val_ptids':int(val_frame.ptid.nunique()),'batch_size':1,'gradient_accumulation':1,'preprocessing':'canonical RAS + official reader-space + fixed bilateral 192^3 crop','prompts':'8 fixed canonical prompts','loss':'FP32 Dice+BCE','amp_forward':False,'grad_scaler':False,'clip_norm':1.0,'weight_decay':1e-5,'parameter_groups':counts,'canonical_trainable_parameters':81930624,'train_manifest_sha256':sha256(train_path),'val_manifest_sha256':sha256(val_path)}; (out/'config.json').write_text(json.dumps(config,indent=2)); pd.DataFrame([{'module_group':g,'parameter_count':n,'requires_grad':True} for g,n in counts.items()]).to_csv(out/'parameter_summary.csv',index=False)
    opt_groups=[{'params':groups['cross_attention_lora'],'lr':1e-4}]
    opt_groups += [{'params':groups['projection_adapter'],'lr':1e-5},{'params':groups['decoder_stages'],'lr':1e-5}]
    opt=torch.optim.AdamW(opt_groups,weight_decay=1e-5); scaler=torch.amp.GradScaler('cuda',enabled=False)
    ...
    while len(steps)<a.max_steps:
        ...
        for idx in order:
            ...
            for j,prompt in enumerate(PROMPTS):
                ...
                with torch.autocast('cuda',enabled=False): logits=w(image,emb)
                ...
                scaler.scale(loss/len(PROMPTS)).backward(); lv.append(float(loss.detach().cpu())); del emb,logits,loss
            scaler.unscale_(opt); ga,params=grad_stats(groups)
