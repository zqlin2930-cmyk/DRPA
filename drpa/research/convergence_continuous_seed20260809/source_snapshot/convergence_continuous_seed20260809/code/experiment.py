...
def preflight():
    ...
    for mod in list(sys.modules.values()):
        ...
        if p and str(p).startswith(str(BASE)) and Path(p).suffix=='.py':source_paths.add(Path(p))
...
def train(m,precision):
    ...
    config={'model':DISPLAY[m],'seed':SEED,'batch_size':1,'max_updates':12000,'required_steps':STEPS,
            ...
            'precision':'strict_FP32_no_autocast_no_gradscaler_no_TF32','activation_checkpointing':m!='FullFT',
