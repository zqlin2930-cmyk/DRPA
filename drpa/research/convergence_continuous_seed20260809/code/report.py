...
def check_inputs(out):
    ...
    for key,model in MODELS.items():
        ...
        assert continuity.optimizer_id.nunique()==1 and continuity.model_id.nunique()==1
...
def main(out):
    ...
    for a in audits:
        ...
        lines += ['',f"## {a['model']}",f"- Seed: {c['seed']}; train337 PTIDs/971 visits; validation85 PTIDs/247 visits; overlap0.",
            ...
            '- Start0; final12000; optimizer continuous yes (same process/object and matching AdamW counters); optimizer resets0; scheduler absent throughout (resets0).',
    ...
    lines += ['',f"Overall acceptance: {'PASS' if fullpass else 'FAIL; inspect undefined metric counts'}."]
