...
def train(m,precision,resume_step=None):
    ...
    if resume_step is None:
        ...
        assert m=='DRPA' and resume_step in STEPS and resume_step>0
    ...
    if resume_step is None:atomic_json(out/'config.json',config)
    else:assert config==json.loads((out/'config.json').read_text()),'Training configuration changed'
    ...
    resume_state=None;elapsed=0.0
    if resume_step is not None:
        resume_state,elapsed=state_tools.initialize_resume(sys.modules[__name__],wrapper,optimizer,out,resume_step,config)
    atomic_json(out/'validation_implementation_v2.json',{'version':fast_validation.VERSION,'first_new_endpoint':0 if resume_step is None else resume_step+1500,'workers':4,'required_metrics_unchanged':True})
    start=time.perf_counter()-elapsed;step=resume_step or 0
    ...
    def endpoint():
        ...
        rows=fast_validation.evaluate(f'{DISPLAY[m]}@{step:05d}',wrapper,val_records,val_ds,cache,shared.evaluator,workers=4)
        rows.to_csv(out/f'validation_raw_step_{step:05d}.csv',index=False)
    ...
    try:
        training_mode()
        if resume_step is None:endpoint()
        else:
            restore_rng(resume_state['rng'])
            assert state_tools.tree_equal(rng_state(),resume_state['rng'])
            progress('RESUMED_EXACT_STATE',next_step=step+1)
        for expected_step,epoch,offset,index,case in order_rows(frame):
            if resume_step is not None and expected_step<=resume_step:continue
            ...
            if resume_step is not None and step==resume_step+1:
                state_tools.verify_first_update(sys.modules[__name__],wrapper,optimizer,losses,norm,case,out,step)
        ...
        atomic_json(out/'complete.json',{'status':'COMPLETE','final_step':step,'optimizer_continuous':True,
            ...
            'wall_clock_time_sec':time.perf_counter()-start,'exact_resume_count':int(resume_step is not None),'runtime_segments':1+int(resume_step is not None),**final})
        progress('COMPLETE')
...
if __name__=='__main__':
    ...
    elif a.model:train(a.model,a.precision,a.resume_step)
