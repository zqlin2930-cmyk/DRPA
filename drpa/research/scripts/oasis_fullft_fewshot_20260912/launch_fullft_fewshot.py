...
import sys
import time
from run_fullft_fewshot import BASE,OUT,TARGET,guarded,sha,atomic_json
...
def manager():
    ...
    for stage in ['train','evaluate','summary']:
        ...
        command=[sys.executable,'-u',str(Path(__file__).parent/'run_fullft_fewshot.py'),stage]
        if stage=='summary': command=[sys.executable,'-u',str(Path(__file__).parent/'summarize_fullft_fewshot.py')]
        with (OUT/f'{stage}.log').open('x') as log:
            ...
            while child.poll() is None:
                atomic_json(OUT/'queue_state.json',dict(status='RUNNING',phase=stage,model='FullFT',pid=child.pid,
